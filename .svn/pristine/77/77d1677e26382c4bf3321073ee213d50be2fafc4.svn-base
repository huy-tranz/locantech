...
// ── DB ↔ UI role mapping ─────────────────────────────────
type UiRole = "Quản trị viên" | "Nhân viên";
const uiToDb: Record<UiRole, string> = { "Quản trị viên": "ADMIN", "Nhân viên": "CUSTOMER" };
const dbToUi: Record<string, UiRole> = { ADMIN: "Quản trị viên", CUSTOMER: "Nhân viên", SUPERADMIN: "Quản trị viên" };
...
export default function AdminUsersPage() {
  ...
  const openCreate = () => {
    setEditing({ id: undefined, name: "", email: "", phone: "", role: "Nhân viên", password: "" });
    setDialogOpen(true);
  };
      ...
      <Card>
        <CardContent className="p-0 overflow-x-auto">
          <Table>
            ...
            <TableBody>
              {users.map((u) => {
                const uiRole = dbToUi[u.role] ?? "Nhân viên";
                return (
                  <TableRow key={u.id}>
