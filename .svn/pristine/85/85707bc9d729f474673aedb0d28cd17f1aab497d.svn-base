    ...
    >
      <div className="bg-card rounded-lg border shadow-sm overflow-hidden">
        <div className="bg-primary px-4 py-2.5">
          <h3 className="text-sm font-bold text-primary-foreground">Danh mục sản phẩm</h3>
        </div>
      ...
      {activeCategory?.children && (
        <div className="absolute left-full top-0 bottom-0 z-30 flex items-stretch">
          {/* Cầu nối full chiều cao sidebar: mục dưới cùng nằm thấp hơn panel — tránh vùng chết khi rê ngang */}
                ...
                <div key={group.id} className={`min-w-0 ${getColumnSpan(group)}`}>
                  ...
                  {hasChildren(group) ? (
                    ...
                    <Link
                      ...
                      className="block leading-6 text-foreground/90 hover:text-primary transition-colors"
                    >
                      {group.name}
                    </Link>
                  )}
